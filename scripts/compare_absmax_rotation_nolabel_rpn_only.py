...
def visualize_comparison(orig_absmax, rot_absmax, rotation_pairs, layer_idx, head_idx,
    ...
    for ax_idx, (ax, absmax, title_prefix, base_color) in enumerate([
        ...
        (axes[1], rot_absmax, 'RPN-only Model', 'forestgreen'),
    ]):
        # Base bars
...
def main():
    ...
    parser.add_argument('rotation_csv', help='Rotation CSV file')
    ...
    parser.add_argument('--post-rope', action='store_true',
                        help='Label as Post-RoPE in title (data always read from pre slot)')
    parser.add_argument('--head-dim', type=int, default=128, help='Head dimension')
    parser.add_argument('--no-all-pairs-when-empty', action='store_true',
                        help='Disable fallback that colors all pairs when CSV has no pairs')
    ...
    # Original nofuse file: post-RoPE data is stored in pre slot (DUMP_PREFIX convention)
    # So always read original from pre slot.
    orig_vals = orig_data['pre_absmax'][layer_idx][head_idx]

    # Rotated file: read from slot based on how it was generated
    if args.post_rope:
        rot_vals = rot_data['post_absmax'][layer_idx][head_idx]
        # fallback: if post slot is all zeros, use pre slot
        if rot_vals.max() == 0:
            print("  ⚠ Rotated post slot is empty, falling back to pre slot")
            rot_vals = rot_data['pre_absmax'][layer_idx][head_idx]
    else:
        rot_vals = rot_data['pre_absmax'][layer_idx][head_idx]
    ...
    if not pairs:
        ...
        if not args.no_all_pairs_when_empty:
            n_pairs = len(orig_vals) // 2
            start_pair = min(32, n_pairs)
            pairs = [(p, 0.0) for p in range(start_pair, n_pairs)]
            print(f"  -> Fallback enabled: using late pairs ({start_pair}..{n_pairs - 1}) for per-pair coloring")
