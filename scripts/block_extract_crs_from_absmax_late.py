...
def extract_static_outliers(data, top_k=4, dim_min=64, beta=4.0, max_scale=None, alpha=1.0, verbose=True):
    """
    Extract static outlier channels (LATE DIMENSIONS ONLY, NO PAIRS).
    Select top-k individual outlier dimensions per block without RoPE pair logic.
    
    Args:
        top_k: Number of individual outlier dimensions per block (not pairs!)
        dim_min: Minimum dimension to consider (default 64 for late dims)
    ...
    # Calculate which blocks are eligible (late dimensions only)
    first_block = dim_min // BLOCK_SIZE
    n_blocks = (n_dims + BLOCK_SIZE - 1) // BLOCK_SIZE
    
    # Max outliers = top_k dimensions * eligible_blocks (NO PAIRS!)
    eligible_blocks = n_blocks - first_block
    max_outliers = top_k * eligible_blocks
    
    results = {
        'n_layers': n_layers,
        'n_heads': n_heads,
        'n_dims': n_dims,
        'top_k': max_outliers,  # Store total individual dimensions
        'dim_min': dim_min,
        'beta': beta,
        'alpha': alpha,
        'outliers': []
    ...
    print(f"\n=== Per-Block Outlier Selection (Late Dims, No Pairs) ===")
    print(f"Strategy: Top-{top_k} individual dimensions per block (dims >= {dim_min})")
    print(f"Blocks: {first_block} to {n_blocks-1} ({eligible_blocks} blocks)")
    print(f"Total outliers: {max_outliers} dimensions")
    if alpha != 1.0:
    ...
    for layer_idx in range(n_layers):
        ...
        for head_idx in range(n_heads):
            ...
            for block_idx in range(first_block, n_blocks):
                ...
                # Find top_k individual dimensions within this block (NO PAIRS!)
                dim_values = [(block_absmax[i], b_start + i) for i in range(len(block_absmax))]
                ...
                # Calculate target value (k+10)-th dimension or last available
                if len(dim_values) > top_k + 10:
                    target_val = dim_values[top_k + 10][0]
                elif len(dim_values) > top_k:
                    target_val = dim_values[-1][0]
                else:
                    target_val = 1.0
                ...
                # Add selected dimensions to total lists
            ...
            if verbose and layer_idx % 8 == 0 and head_idx == 0:
                ...
                print(f"    Selected {len(outlier_indices[outlier_indices >= 0])} outlier dimensions across {eligible_blocks} blocks")
                # Show first few dimensions per block
                for block_idx in range(first_block, min(first_block + 2, n_blocks)):
                    block_dims = [idx for idx in outlier_indices if block_idx * BLOCK_SIZE <= idx < (block_idx + 1) * BLOCK_SIZE]
                    if block_dims:
                        dims_str = ", ".join([str(d) for d in block_dims[:6]])
                        print(f"      Block {block_idx} [{block_idx*BLOCK_SIZE}-{(block_idx+1)*BLOCK_SIZE-1}]: {dims_str}")
...
def save_static_scales(results, output_path):
    ...
    Format: k = number of individual outlier dimensions (top_k)
    ...
    print(f"\nSaved static CRS scales (late dims, no pairs) to: {output_path}")
    print(f"  File size: {Path(output_path).stat().st_size} bytes")
    print(f"  Format: k={results['top_k']} individual outlier dimensions per head")


def main():
    parser = argparse.ArgumentParser(description='Extract per-block CRS scales from absmax values (LATE DIMS ONLY, NO PAIRS)')
    ...
    parser.add_argument('--top-k', type=int, default=4, help='Number of individual outlier dimensions per block (not pairs!)')
    parser.add_argument('--dim-min', type=int, default=64, help='Minimum dimension index to consider (late dimensions)')
