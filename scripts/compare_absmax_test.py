...
def visualize_comparison(orig_absmax, crs_absmax, outlier_indices, outlier_scales,
    ...
    if valid_indices:
        ...
        for idx, scale in zip(outlier_indices, outlier_scales):
            ...
            # Only label dimensions 82 and 83
            if idx not in [78, 79]:
                continue
    ...
    if valid_indices:
        ...
        for idx, scale in zip(outlier_indices, outlier_scales):
            ...
            # Only label dimensions 70, 78, 79
            if idx not in [70, 78, 79]:
                continue
        ...
        # Add label for dim 70 (non-outlier)
        if 70 < n_dims:
            idx = 70
            y_new = crs_absmax[idx]
            
            # Add blue dot for dim 70
            ax2.scatter([idx], [y_new], color='blue', s=50, zorder=3, edgecolors='darkblue', linewidths=1.2)
