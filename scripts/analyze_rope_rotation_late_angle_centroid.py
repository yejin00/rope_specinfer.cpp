...
def classify_shape_orientation_table(
    ...
    """
    if shape_base not in ('pre', 'post'):
        raise ValueError(f"Invalid shape_base: {shape_base}")
    if orientation_base not in ('pre', 'post'):
        raise ValueError(f"Invalid orientation_base: {orientation_base}")
    ...
    for key, group in grouped.items():
        spreads = np.array([g[f'{shape_base}_spread'] for g in group], dtype=np.float64)
        anisos = np.array([g[f'{shape_base}_anisotropy'] for g in group], dtype=np.float64)
        compacts = np.array([g[f'{shape_base}_mean_compactness'] for g in group], dtype=np.float64)
        ...
        for g in group:
            S = g[f'{shape_base}_spread']
            A = g[f'{shape_base}_anisotropy']
            C = g[f'{shape_base}_mean_compactness']
            ...
            # --------------------------------------------
            # centroid-only rotation: use mean angle for all pairs
            # --------------------------------------------
            orientation_source = f'{orientation_base}_mean'
            base_angle = g[f'{orientation_base}_mu_angle']
...
def save_shape_orientation_csv_named(rows, output_dir, csv_name):
...
def print_shape_orientation_summary(rows, topk=10):
    ...
    for r in rows[:topk]:
        shape_source = r.get('shape_source', 'pre')
        spread = r.get(f'{shape_source}_spread', np.nan)
        aniso = r.get(f'{shape_source}_anisotropy', np.nan)
        mcompact = r.get(f'{shape_source}_mean_compactness', np.nan)
        print(
...
def main():
    parser = argparse.ArgumentParser(description='RoPE Shape / Orientation CSV Export (centroid-only angles)')
    ...
    parser.add_argument('--shape-base', choices=['pre', 'post'], default='pre', help='Base distribution for shape classification')
    parser.add_argument('--orientation-base', choices=['pre', 'post'], default='pre', help='Base centroid angle for suggested alpha')
    parser.add_argument('--output-csv-name', default=None, help='Output CSV filename')
    ...
    print("=" * 60)
    print("RoPE Shape / Orientation CSV Export (centroid-only angles)")
    print("=" * 60)
    print(f"  shape_base={args.shape_base}, orientation_base={args.orientation_base}")
    ...
    shape_rows = classify_shape_orientation_table(
        ...
        layers=args.layers,
        heads=args.heads,
        max_points=args.max_tokens,
        pair_start=args.pair_start,
        pair_end=args.pair_end,
        diag_thresh_deg=args.diag_thresh_deg,
        axis_thresh_deg=args.axis_thresh_deg,
        shape_base=args.shape_base,
        orientation_base=args.orientation_base,
    )
    if args.output_csv_name is not None:
        csv_name = args.output_csv_name
    elif args.shape_base == 'post' and args.orientation_base == 'post':
        csv_name = 'shape_orientation_table_post.csv'
    elif args.shape_base == 'pre' and args.orientation_base == 'pre':
        csv_name = 'shape_orientation_table_pre.csv'
    else:
        csv_name = f'shape_orientation_table_shape_{args.shape_base}_orient_{args.orientation_base}.csv'

    save_shape_orientation_csv_named(shape_rows, args.output_dir, csv_name)
