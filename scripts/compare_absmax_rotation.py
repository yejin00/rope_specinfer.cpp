...
def visualize_comparison(orig_absmax, rot_absmax, rotation_pairs, layer_idx, head_idx,
    ...
    for ax_idx, (ax, absmax, title_prefix, base_color) in enumerate([
        ...
        for (hf_d0, hf_d1, pidx, adeg, pc) in pair_dims:
            ...
            # Label at hf_d0
            level = used_positions.get(hf_d0, 0)
            y_shift = level * 1.2
            used_positions[hf_d0] = level + 1
            ...
            label = f'p{pidx}\n({hf_d0},{hf_d1})\n{adeg:.1f}°'
            ax.text(
                hf_d0, y0 + 0.3 + y_shift, label,
                fontsize=6, ha='center', weight='bold', color='black',
                bbox=dict(boxstyle='round,pad=0.12', facecolor=pc, alpha=0.6, lw=0)
            )
