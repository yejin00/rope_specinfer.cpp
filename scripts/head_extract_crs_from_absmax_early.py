...
def extract_static_outliers(data, top_k=8, dim_min=0, dim_end=64, beta=4.0, max_scale=None, alpha=1.0, verbose=True):
    """
    Extract static outlier channels using Per-Head Strategy (pair-wise).
    
    Args:
        top_k: Number of outlier PAIRS per head
    ...
    # Max outliers = top_k pairs * 2 dims/pair per head
    max_outliers = top_k * 2
    
    results = {
        'n_layers': n_layers,
        'n_heads': n_heads,
        'n_dims': n_dims,
        'top_k': top_k,  # Store number of pairs for header
        'dim_min': dim_min,
        'dim_end': dim_end,
        'beta': beta,
        'alpha': alpha,
    ...
    print(f"\n=== Per-Head Outlier Selection (Early Dims, Pair-wise) ===")
    print(f"Strategy: Top-{top_k} pairs per head (dims {dim_min} to {dim_end-1})")
    print(f"Dimension range: [{dim_min}, {dim_end})")
    print(f"Total outliers per head: {max_outliers} dims ({top_k} pairs)")
    if alpha != 1.0:
    ...
    for layer_idx in range(n_layers):
        ...
        for head_idx in range(n_heads):
            ...
            # Get absmax values for early dimensions only (dim_min to dim_end)
            early_absmax = original_absmax[dim_min:dim_end]
            
            # Find top_k pairs from the entire early dimension range
            ...
            for i in range(0, len(early_absmax), 2):
                if i + 1 < len(early_absmax):
                    d_even = dim_min + i
                    d_odd = dim_min + i + 1
                    pair_max = max(early_absmax[i], early_absmax[i+1])
                    pair_maxes.append((pair_max, d_even, d_odd))
            ...
            # Calculate target value using median of all pairs
            if len(pair_maxes) > 0:
                mid_idx = len(pair_maxes) // 2  # Middle index
                target_val = pair_maxes[mid_idx][0]
            else:
                target_val = 1.0
            ...
            for pair_max, d_even, d_odd in selected_pairs:
                prs_indices.extend([d_even, d_odd])  # even and odd dims
                
                # Calculate scale needed to bring this outlier down to 'target_val'
                raw_scale = pair_max / target_val
            ...
            if verbose and layer_idx % 8 == 0 and head_idx == 0:
                valid_indices = prs_indices[prs_indices >= 0]
                print(f"  Layer {layer_idx}, Head {head_idx}:")
                print(f"    Selected {len(selected_pairs)} pairs from range [{dim_min}, {dim_end})")
                if len(selected_pairs) > 0:
                    pairs_str = ", ".join([f"({p[1]},{p[2]})" for p in selected_pairs[:10]])
...
def main():
    parser = argparse.ArgumentParser(description='Extract per-head CRS scales from absmax values (early dims, pair-wise)')
    ...
    parser.add_argument('--top-k', type=int, default=8, help='Number of outlier PAIRS per head')
