...
def extract_static_outliers(data, top_k=2, dim_min=0, dim_end=64, beta=4.0, max_scale=None, alpha=1.0, verbose=True):
    """
    ...
    last_block = (dim_end + BLOCK_SIZE - 1) // BLOCK_SIZE
    
    # Max outliers = top_k pairs * 2 dims/pair * eligible_blocks
    eligible_blocks = last_block - first_block
    ...
    print(f"Strategy: {top_k} pairs per block (dims {dim_min} to {dim_end-1})")
    print(f"Blocks: {first_block} to {last_block-1} ({eligible_blocks} blocks)")
    ...
    for layer_idx in range(n_layers):
        ...
        for head_idx in range(n_heads):
            ...
            for block_idx in range(first_block, last_block):
                b_start = max(block_idx * BLOCK_SIZE, dim_min)
                b_end = min((block_idx + 1) * BLOCK_SIZE, dim_end)
                ...
                for p in selected_pairs:
                    ...
                    d_even_idx = p[1]
                    d_odd_idx = p[2]
                    prs_indices.extend([d_even_idx, d_odd_idx])  # even and odd dims
                    
                    # Calculate L2 norm of the pair as scale
                    even_val = original_absmax[d_even_idx]
                    odd_val = original_absmax[d_odd_idx]
                    l2_norm = np.sqrt(max(even_val, odd_val))
                    final_scale = max(1.0, l2_norm)
            ...
            if verbose and layer_idx % 8 == 0 and head_idx == 0:
                ...
                for block_idx in range(first_block, min(first_block + 2, last_block)):
                    b_start = max(block_idx * BLOCK_SIZE, dim_min)
                    b_end = min((block_idx + 1) * BLOCK_SIZE, dim_end)
                    block_pairs = [p for p in all_pairs if b_start <= p[1] < b_end]
                    if block_pairs:
                        pairs_str = ", ".join([f"({p[1]},{p[2]})" for p in block_pairs[:3]])
                        print(f"      Block {block_idx} [{b_start}-{b_end-1}]: {pairs_str}")
