...
OUTPUT_ROOT="${OUTPUT_ROOT:-/data/yejin/needle_seed5}"
...
CTX_LENS="${CTX_LENS:-2048,4096,8192,16384,32768,51200,65536}"
...
Runs the 7 NIAH presets sequentially. For each preset:
1. start llama-server with the requested model/KV config
2. wait for /health
3. run scripts/niah.py
4. stop the server

Options:
  --python PATH              Python executable to use
  --repo-root PATH           Repo root containing llama-server and scripts/niah.py
  --server-bin PATH          Explicit llama-server binary path
  --niah-script PATH         Explicit scripts/niah.py path
  ...
  --server-ctx-size N        llama-server -c value; default is max(ctx-lens)
  ...
  --ctx-lens LIST            NIAH context lengths (default: ${CTX_LENS})
  --depths LIST              NIAH insertion depths (default: ${DEPTHS})
  --seeds LIST               NIAH seeds (default: ${SEEDS})
  --needle-kind KIND         passkey|word|kv (default: ${NEEDLE_KIND})
  --digits N                 digits for passkey mode (default: ${DIGITS})
  --format MODE              chat|completion (default: ${FORMAT_MODE})
  --system-prompt TEXT       System prompt for chat mode
  --max-output-tokens N      Generation budget for the answer (default: ${MAX_OUTPUT_TOKENS})
  --temperature X            Generation temperature (default: ${TEMPERATURE})
  --timeout SEC              HTTP timeout for NIAH client (default: ${TIMEOUT})
  --ready-timeout SEC        Server readiness timeout (default: ${READY_TIMEOUT})
  --filler-file PATH         Optional filler file for NIAH
  --no-verbose               Disable per-trial NIAH logging
  --cache-prompt             Enable llama-server prompt caching in NIAH
...
if [[ -z "${SERVER_CTX_SIZE}" ]]; then
    SERVER_CTX_SIZE="$("${PYTHON_BIN}" - "${CTX_LENS}" <<'PY'
import sys
values = [int(part.strip()) for part in sys.argv[1].split(",") if part.strip()]
if not values:
    raise SystemExit("ctx-lens must not be empty")
print(max(values))
PY
)"
fi
...
run_eval() {
    ...
    if [[ "${CACHE_PROMPT}" -eq 1 ]]; then
        niah_cmd+=(--cache-prompt)
    fi
...

{
    ...
    echo "NIAH args: ctx_lens=${CTX_LENS} depths=${DEPTHS} seeds=${SEEDS} needle_kind=${NEEDLE_KIND} format=${FORMAT_MODE} filler_file=${FILLER_FILE} max_output_tokens=${MAX_OUTPUT_TOKENS} temperature=${TEMPERATURE} verbose=${VERBOSE}"
